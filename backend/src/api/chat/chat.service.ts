...
export class ChatService {
  ...
  private async participantCheckFromFriendID(
    userId: string,
    friendId: string,
  ): Promise<string> {
    const chat = await this.prisma.dMChat.findFirst({
      where: {
        OR: [
  ...
  async fetchDMHistoryGDPR(
    userId: string,
    friendId: string,
  ): Promise<{ chatId: string; messages: GDPRDMHistory }> {
    const chatId = await this.participantCheckFromFriendID(userId, friendId);
    const raw = await this.findDMMessages(chatId);
    const messages = raw.map(({ id, date, message, sender }) => ({
      id,
      date,
  ...
  async fetchLobbyHistoryGDPR(): Promise<GDPRLobbyHistory> {
    const raw = await this.findLobbyMessages();
    const messages = raw.map(
      ({ id, date, message, moderated, senderId, sender }) => ({
        id,
        moderated,
        rank: !sender || senderId == 'Guest' ? null : sender.rank,
        date,
        senderId: senderId ?? 'Guest',
        username: sender ? sender.username : 'Guest',
        message,
      }),
    );
    return messages;
